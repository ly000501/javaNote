...
​		微服务之间在相互调用时关系错综复杂，一个请求可能需要同时调用多个微服务接口才能实现某一个功能，会形成非常复杂的调用链路：

![img](assets\2020-01-06-1.png)

如果在同时调用多个微服务的过程中其中一个微服务出现了问题，就可能发生雪崩现象.如图，一次业务请求需要调用A、P、H、I四个服务，这四个服务又可能调用其它服务。如果此时，某个服务出现了异常：

![img](assets\874963-20180730172821821-960520983.png)

请求就会发生阻塞，用户不能得到服务器的响应，tomcat的这个线程也不会被释放，当越来越多的用户请求到来，越来越多的线程就会被阻塞：

![img](assets\874963-20180730172949326-29467411.png)

服务器支持的线程和并发数有限，请求一直阻塞，会导致服务器资源迅速耗尽，从而导致多个服务同时都不可用，形成雪崩效应。

形成雪崩效应的原因是：服务提供者不可用->服务调用者请求重试->服务调用者所在的系统资源耗尽，服务调用者不可用。

​	这就好比一条电脑生产线，生产不同的电脑需要使用不同的零件。如果某个关键零件不能正常使用，那么整台电脑就无法装配。知道零件正常，才能继续组装。如果多条生产线同时有多台电脑都需要这个零件，那么整个工厂都将陷入等待的状态，导致整个工厂的生产陷入瘫痪。由一个零件产生的问题会波及整个工厂的正常生产。
...
![img](assets\874963-20180730173442729-1485668320.png)

解读：

​	Hystix为每个依赖项维护一个小的线程池(或信号量)；如果线程池满了，那么该依赖性将会立即拒绝请求，而不是让请求排队。从而加速判定请求失败的时间。

​	用户的请求将不再直接访问服务，而是通过线程池中的空闲线程来访问服务，如果**线程已满，或者请求超时**，则会降级处理。

[^服务的降级：优先保证核心服务的运行，而非核心服务可停止服务或减弱服务。]: 

当用户的请求发生故障时，请求不会被阻塞，更不会无休止的等待服务器响应或者等到系统崩溃。至少可以看到一个执行结果（比如返回一个友好的提示信息）。

​	服务降级虽然会导致请求失败，但是不会影响和这个请求相关的其它服务。
...
》open:打开状态，所有的请求都会被降级。Hystrix会对请求情况计数，当一定时间内失败请求百分比达到阈值，则触发熔断，断路器就会完全打开。默认阈值的百分比是50%，请求次数不能少于20次。



》Half open:半开状态，open状态不是永久的，打开之后就会进入休眠（默认5S），随后断路器就会自动进入半开状态。如果这些请求都没有问题，则会完全关闭断路器，否则继续保持打开状态。
...
```
@EnableCircuitBreaker  //在主方法上添加此注解，开启断路器
```
